...
- `git add .`と`git add -A`を使わず、stage対象のpathを明示する。commit前にstaged file名、staged diff、`git diff --cached --check`、working treeの`git diff --check`、secret・実データ・無関係な変更の不在を確認する。
- commit messageは変更の責務を短く表す日本語にする。
- ソースコード変更依頼はcommit、push、非ドラフトPR作成・更新、CI再実行、reviewへの返信・修正、対応済みthreadの解決までを許可する。
...
- latest headに紐づく対象branchのCIを確認し、成功後はlatest-head review、未解決thread、mergeabilityも確認する。失敗時は原因を特定し、変更範囲内なら修正、commit、push、再確認する。
- 開発中とreview修正中は変更pathに対応するfocused testを使い、最終HEAD確定前にfull gateを機械的に繰り返さない。
- 配送対象の最終HEADでは、変更範囲に必要な検証を入力閉包へ束縛して一度実行する。ガバナンス変更では `node scripts/validate-governance.mjs` を使い、同じsnapshot・条件の検査を重ねない。stacked PRは親merge後にbase統合、必要な検証、latest HEAD reviewを確認する。
- workflowまたはpath classifierを変更した場合は、変更pathに対応するcontract test、変更workflowのYAML parse、`base...head` classification、latest Actionsを選択する。frontend / Rust / native runtimeに影響しない場合、無関係なPlaywrightやnative E2Eを追加しない。workflow未変更のreview fixでは、既存のYAML証跡を保持する。
...
- 同じHEAD・入力閉包・条件で成功したgateは再実行しない。新commitだけではlocal full gateを一括失効させず、閉包と交差する変更だけを失効させる。閉包が同じ証拠を後続HEADで再利用する場合は、由来HEADと新しいHEADをledgerへ併記する。
...
- merge直前は再確認済みの単一snapshotへlatest HEAD、base、CI、latest-head review、未解決thread、mergeabilityを記録する。snapshot後にHEAD・base・CI・review状態が変わった場合は、該当証拠を失効して更新する。最終delivery judgmentはprimaryがacceptance、CI、review、thread、mergeabilityを照合して行う。
