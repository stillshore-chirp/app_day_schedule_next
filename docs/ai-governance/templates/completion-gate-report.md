...
## Day Schedule Nextの製品適合

- 製品目的・不変条件への接続（[product-invariants.md](../../product-invariants.md)）:
- 影響するアプリ本体UI surface / state:
- 影響する製品契約（時間、同期、通知、Ticket、backup / restore）:
- native / OS証跡、未確認のOS・permission・実データ境界:
- GitHub共同作業面だけの場合に適用した比例的な文言・構造・link・公開安全性確認:
