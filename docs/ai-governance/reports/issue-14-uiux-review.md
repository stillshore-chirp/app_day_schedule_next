...
- Native execution: E2E fixtureをOS付属`sqlite3` CLIからE2E feature限定のSQLx commandへ置換した。Compact windowはWDIO公式label APIだけで切り替え、mainへ戻ってからE2E feature限定commandで閉じる。E2E configだけcompactへWDIO権限を付与し、通常appのcapabilityは変更しない。全3 spec / 14 testとGoogle関連2 testの独立実行がPass
