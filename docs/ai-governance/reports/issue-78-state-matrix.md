...
| 辺の drag            | drag 中も内容領域 1:1                      | 余白を作らず時計を拡大縮小 | 左 / 右 / 上 / 下            | 反対方向へ drag         | OS 標準 resize                     | AppKit query / Windows native build + unit        | 制約登録済、実 drag 未実施 |
| 角の drag            | drag 中も内容領域 1:1                      | 自然な対角 resize          | 4 隅                         | 反対方向へ drag         | OS 標準 resize                     | AppKit query / Windows native build + Rust 8 方向 | 制約登録済、実 drag 未実施 |
| 旧寸法の復元         | 保存済み非正方形を短辺基準の正方形へ補正   | 更新直後から比率を保証     | 時計を開く / 再表示          | 4 段階 size / drag      | 自動補正                           | native constraint query                           | macOS 済、Windows build 済 |
...
| 最大化               | 最大化操作を無効化                         | 非正方形化を防ぐ           | OS window control            | 通常 resize             | disabled native control            | macOS / Windows native build                      | 済                         |
...
Windows x64 の native workflow ではアプリの build とアナログ時計の E2E case が通過しました。workflow 全体は今回と無関係なテンプレート編集ボタンの探索失敗で failure になったため、正方形制約の合格と suite 全体の状態を分けて扱います。Windows の手動 drag と Snap は未実施です。
