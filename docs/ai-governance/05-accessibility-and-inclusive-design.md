...
## Day Schedule Nextのdesktop適用

Day Schedule Nextのアプリ本体UIでは、Tauri WebViewだけでなく、macOS / Windowsのwindow、menu、Compact Window、notification、OS scaleを対象に、該当する主要taskを確認します。

- schedule block、timeline、current-time line、overlap、conflict、sync statusは、title、start / end、duration、selected、current、scopeを必要な粒度で名前・状態として伝える。色・位置・動きだけに依存しない。
- dragによる作成、move、resizeには、direct time input、keyboard command、context actionなどの等価経路を用意し、保存精度1分と表示snapの違いを理解できるようにする。
- Today、Inspector、Ticket / Kanban、Template、Focus、Sync / Conflict、Data画面では、作成・編集・削除・Undo / Redo・retry・restore previewなどの主要taskについて、entryから完了または回復までのkeyboardとfocusを確認する。
- local save、sync pending、conflict、permission、notification delivery、Focus phaseの更新は、適切な粒度のstatusとして通知する。毎秒の時計・countdownをlive regionへ流さない。
- 200% text / OS scale、720px相当の狭幅、high contrast / forced colors、reduced motion、長い予定名・メモ・エラーメッセージで、主操作と回復導線が失われないかを確認する。
- 実施できないOS、screen reader、native notificationの確認は未確認項目とし、WebViewの静的検査やbrowser previewを実機証跡へ格上げしない。

製品固有の保存、時間、同期、通知、migrationの条件は [docs/product-invariants.md](../product-invariants.md) と各domain Skillを正本とし、本書ではaccessibility上の観察点だけを定義します。
