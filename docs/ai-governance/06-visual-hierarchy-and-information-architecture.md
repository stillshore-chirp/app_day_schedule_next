...
## 17. Day Schedule Nextの情報階層

Day Schedule Nextで最初に見えるべき情報は、次の順序を基準にします。実際の画面・状態・利用文脈による例外は、観測事実とtask影響を添えて判定します。

1. Todayの日付、表示範囲、現在地、予定作成などの主操作。
2. current / next / remaining、active Focus、重大なconflict / auth状態。
3. detail timelineと選択中scheduleの時間・duration・編集操作。
4. overview、free time、filter、category、Ticket context。
5. sync summary、notification、secondary utility。
6. diagnostics、advanced setting、履歴・補助情報。

24時間overviewは一日の分布と重なりを、detail timelineは分単位編集を支援します。同じentityの選択・表示範囲・状態がoverviewとdetailで食い違わず、overlapや日跨ぎで予定のidentityが隠れないことを確認します。Compact WindowやNow Dockはcurrent / next / remainingを優先し、main windowの編集情報を過剰に複製しません。

日付、時刻、duration、件数、progressは符号・単位・桁を含めて読み取れるようにし、23:59、日跨ぎ、複数current、500件、長い日本語で、選択中・重大状態・主操作が埋もれないことを確認します。macOS / Windowsのtitlebar、menu、scale、window boundsの差で、同じtaskの階層やfocus位置を壊さないことも対象です。
